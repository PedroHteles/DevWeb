...
router.post("/addcarrinho", async (req, res) => {
  const eMail = (req.userJWT.eMail)
  const {idProduto,metodo} = (req.body)
  console.log(idProduto,metodo)
  let quantidade = 1
  const pesquisaCarrinho = await Carrinho.findOne({ where: {eMail,idProduto} }).catch((err) =>{console.log(err);});
  if(!pesquisaCarrinho){
    try {
      await Carrinho.create({ idProduto, eMail, quantidade});
    ...
    if(metodo === true){
      ...
      try {
        await Carrinho.update({quantidade},{where: {eMail,idProduto}});
        const resp = await Carrinho.findAll({ where: {eMail}})
      ...
      try {
        if(quantidade >= 0){
          ...
          await Carrinho.destroy({where: {eMail,idProduto}});
          const resp = await Carrinho.findAll({ where: {eMail}})
          return res.json(resp);
        }
      } catch (error) {
